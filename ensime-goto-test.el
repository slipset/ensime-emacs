...
With an argument, open the test file in another window."
  ...
  (block nil
    (when (ensime-is-test-file buffer-file-name)
      (return (message "This isn't an implementation file")))
    (let* ((impl-class
...
implementation class. With an argument, open the test file in another window."
  ...
  (block nil
    (unless (ensime-is-test-file buffer-file-name)
      (return (message "This isn't a test file")))
    (let* ((test-class
            (or (ensime-top-level-class-closest-to-point)
                (return (message "Could not find top-level class"))))
           (impl-class-info
            (or (ensime-get-impl-class-info test-class)
                (return (message "Could not determine implementation class for %s"
                                 test-class)))))
      (ensime-goto-source-location impl-class-info))))

(defun ensime-get-goto-test-config (key)
  (let* ((module-name (plist-get (ensime-config (ensime-connection)) :project-name))
         (module-params (cdr (assoc module-name ensime-goto-test-configs))))
...
(defun ensime-goto-test--impl-class-name (test-class)
  (let ((suffixes (ensime-get-goto-test-config :test-class-suffixes)))
    (dolist (s suffixes)
      (when (string-match (concat s "$") test-class)
        (return
...
(defun ensime-goto-test--is-test-dir (dir)
  (or
   (string-match "src/test/scala/$" dir)
   (string-match "/tests?/$" dir)))
...
(defun ensime-goto-test--test-class-names-ensime (impl-class)
  "Version of ensime-goto-test--test-class-names for the ensime source code"
  (list
   (replace-regexp-in-string "^org\\.ensime\\..*\\.\\([^.]+\\)$"
                             "org.ensime.test.\\1Spec" impl-class t)))

(defun ensime-goto-test--impl-class-name-ensime (test-class)
  "Version of ensime-goto-test--impl-class-name for the ensime source code"
  ;; The ensime test classes are all in the org.ensime.test package.
  ;; To find an implementation class, we must search all classes in the
  ;; org.ensime package and match based on the class name only.
  (labels
      ((get-members (infos)
        (mapcan
         (lambda (info)
           (cons (plist-get info :full-name)
                 (get-members  (plist-get info :members))))
         infos)))
    (let* ((package-info (ensime-rpc-inspect-package-by-path "org.ensime"))
           (impl-class-name
            (replace-regexp-in-string "^.*\\.\\([^.]+\\)Spec$" "\\1" test-class))
           (package-members (get-members (plist-get package-info :members)))
           (candidate
            (find-if
             (lambda (c) (string-match
                          (concat "^.*\\."
                                  (regexp-quote impl-class-name)
                                  "$")
                          c))
             package-members))
           )
